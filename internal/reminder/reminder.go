...
func LoadReminderManager(filepath string) *ReminderManager {
	...
	targetPath := util.FindDataDir(filepath)
...
func BuildMorningReminder(chatJID string, config *schedule.JadwalConfig, taskManager *task.TaskManager, now time.Time, linkManagers ...*link.LinkManager) string {
	var holiday *schedule.ScheduleOverride
	...
	var pesan string
	if holiday != nil {
		hariIndo := util.GetHariIndonesia(now)
	...
	if taskManager != nil {
		...
		if err == nil && len(urgentTasks) > 0 {
			...
			for _, ut := range urgentTasks {
				badge := task.GetUrgencyBadge(ut.DeadlineAt, now)
...
func (rm *ReminderManager) StartScheduler(
	...
	settingsMgr *chat.ChatSettingsManager,
	taskManager *task.TaskManager,
	linkManagers ...*link.LinkManager,
) {
	go func() {
		ticker := time.NewTicker(30 * time.Second)
		defer ticker.Stop()

		var lm *link.LinkManager
