...
func (j *JadwalConfig) Reload() (string, error) {
	...
	resolvedPath := util.FindDataDir(j.FilePath)
	data, err := os.ReadFile(resolvedPath)
	if err != nil {
...
func (j *JadwalConfig) GetNextClass(currentTime time.Time) string {
	...
	hariIndonesia := util.GetHariIndonesia(currentTime)
	...
	for _, item := range todayItems {
		start, end, err := util.ParseJamRange(item.Jam, currentTime)
		if err != nil {
			continue
		}
...
func (j *JadwalConfig) GetDaftarMatkul() string {
	...
	for _, item := range j.Jadwal {
		...
		if item.InisialDosen != "" && !util.Contains(info.Dosen, item.InisialDosen) {
			info.Dosen = append(info.Dosen, item.InisialDosen)
		}
		if item.Hari != "" && !util.Contains(info.Hari, item.Hari) {
...
func (j *JadwalConfig) GetByHari(hariInput string, refTime ...time.Time) string {
	...
	case "hari ini", "hariini", "today", "now", "":
		hariInput = strings.ToLower(util.GetHariIndonesia(waktuSekarang))
	case "besok", "tomorrow":
		hariInput = strings.ToLower(util.GetHariIndonesia(waktuSekarang.Add(24 * time.Hour)))
	case "jum'at":
		hariInput = "jumat"
	}
...
func (j *JadwalConfig) GetByHariWithOverrides(
	...
	case "jum'at":
		targetDate = util.GetDateForDayName("jumat", waktuSekarang)
	default:
		targetDate = util.GetDateForDayName(hariInput, waktuSekarang)
	}
	...
	for _, o := range overrides {
		if o.Type == "HOLIDAY" {
			...
			hariIndo := util.GetHariIndonesia(targetDate)
	...
	for _, it := range normalItems {
		...
		startTime, _, _ := util.ParseJamRange(it.Jam, targetDate)
	...
	for _, o := range overrides {
		if o.TargetDate == targetDateStr {
			if o.Type == "RESCHEDULE" && (o.OrigDate != targetDateStr || o.OrigJam != o.NewJam) {
				startTime, _, _ := util.ParseJamRange(o.NewJam, targetDate)
				entries = append(entries, RenderScheduleEntry{
					StartTime:    startTime,
					JamDisplay:   o.NewJam,
	...
	sb.WriteString(fmt.Sprintf("📅 *JADWAL %s, %02d %s*\n",
		strings.ToUpper(hariTarget), targetDate.Day(), util.GetBulanIndonesia(targetDate)))
	if banner := j.FormatClassBanner(); banner != "" {
		sb.WriteString(banner)
	}
	sb.WriteString("──────────\n\n")
...
func (j *JadwalConfig) getActiveItemsForDate(targetDate time.Time, scopeJID string, om *OverrideManager) (isHoliday bool, holidayReason string, items []ActiveCandidate) {
	...
	for _, item := range normalItems {
		...
		start, end, err := util.ParseJamRange(item.Jam, targetDate)
		if err != nil {
			continue
		}
...
func (j *JadwalConfig) GetMenu() string {
	...
	sb.WriteString("• `!link` ➔ Tautan penting (Drive/Zoom)\n\n")
	...
	sb.WriteString("• `!drive` / `!zoom` ➔ Tautan materi & daring\n")
...
func (j *JadwalConfig) GetKeywords() string {
	...
	sb.WriteString("5️⃣ *Tautan Penting Kelas:*\n")
	sb.WriteString("• `!link` / `!tautan` ➔ Semua tautan kelas\n")
	sb.WriteString("• `!drive` ➔ Shortcut Google Drive & OneDrive\n")
	sb.WriteString("• `!zoom` / `!gmeet` ➔ Shortcut link kuliah daring\n")
	sb.WriteString("• `!link cari [kata]` ➔ Cari tautan berdasarkan nama/deskripsi\n")
	sb.WriteString("• `!link tambah [Judul] | [URL] (| [Catatan])` (Admin)\n")
	sb.WriteString("• `!link hapus [ID]` (Admin)\n\n")

	sb.WriteString("6️⃣ *Jadwal Pengganti (Khusus Admin):*\n")
	...
	sb.WriteString("7️⃣ *Pengaturan Kelas (Multi-Tenant):*\n")
	...
	sb.WriteString("8️⃣ *Informasi & Pencarian:*\n")
	sb.WriteString("• `!matkul` ➔ Daftar semua mata kuliah\n")
	...
	sb.WriteString("9️⃣ *Pengaturan Admin Lainnya:*\n")
